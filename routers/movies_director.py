...
def create_mov_director(movie_director:Mov_DirectorSchemas):
    ...
    return JSONResponse(content={"menssage": "mov_director created succesfully", "status_code": 201}, status_code= 201)
...
def update_mov_director(id:int,data:Mov_DirectorSchemas):
    ...
    result=movi_directorService(db).get_for_id(id)
    if not result:
        return JSONResponse(content= {"message":"mov_director don't found", "status_code":404})
    movi_directorService(db).update_mov_director(data)
    return JSONResponse(content={"message":"mov_actor updated succesfully", "status_code":200}, status_code=200)

@mov_director_router.delete("/mov_director{id}", tags=["mov_director"])
def delete_mov_director(id:int,data:Mov_DirectorSchemas):
    db=Session()
    result=movi_directorService(db).get_for_id(id)
    if not result:
        return JSONResponse(content= {"message":"mov_director don't gound", "status_code":404})
    movi_directorService(db).delete_mov_director(data)
    return JSONResponse(content={"message":"mov_actor delete succesfully", "status_code":200}, status_code=200)
