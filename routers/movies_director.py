...
from service.movies_director import Mov_Director as mov_directorService
...
def get_mov_director():
    db = Session()
    result = mov_directorService(db).get_mov_director()
...
def get_mov_director_for_id(id: int):
    db = Session()
    result = mov_directorService(db).get_for_id(id)
    return result
...
def create_mov_director(movie_director: Mov_DirectorSchemas):
    db = Session()
    mov_directorService(db).create_mov_director(movie_director)
...
def update_mov_director(id: int, data: Mov_DirectorSchemas):
    ...
    mov_directorService(db).update_mov_director(data)
...
def delete_mov_director(id: int):
    ...
    if not result:
        return JSONResponse(content={"message": "mov_director not found", "status_code": 404})
    mov_directorService(db).delete_mov_director(id)
